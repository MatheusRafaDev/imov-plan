...
function Th({ children, right, className = "" }: { children?: React.ReactNode; right?: boolean; className?: string }) {
  return <th className={`px-2.5 py-2 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground whitespace-nowrap first:pl-4 sm:first:pl-6 md:first:pl-8 lg:first:pl-4 last:pr-4 sm:last:pr-6 md:last:pr-8 lg:last:pr-4 ${right ? "text-right" : "text-left"} ${className}`}>{children}</th>;
}
...
function Td({ children, right, className = "", suppressHydrationWarning }: { children?: React.ReactNode; right?: boolean; className?: string; suppressHydrationWarning?: boolean }) {
  return <td suppressHydrationWarning={suppressHydrationWarning} className={`px-2.5 py-2.5 num text-xs sm:text-sm text-foreground whitespace-nowrap first:pl-4 sm:first:pl-6 md:first:pl-8 lg:first:pl-4 last:pr-4 sm:last:pr-6 md:last:pr-8 lg:last:pr-4 ${right ? "text-right" : "text-left"} ${className}`}>{children}</td>;
}
