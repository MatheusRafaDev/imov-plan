...
function Th({ children, right, className = "" }: { children?: React.ReactNode; right?: boolean; className?: string }) {
  return <th className={`px-2.5 py-2 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground ${right ? "text-right" : "text-left"} ${className}`}>{children}</th>;
}
...
function Td({ children, right, className = "", suppressHydrationWarning }: { children?: React.ReactNode; right?: boolean; className?: string; suppressHydrationWarning?: boolean }) {
  return <td suppressHydrationWarning={suppressHydrationWarning} className={`px-2.5 py-2.5 num text-xs sm:text-sm text-foreground ${right ? "text-right" : "text-left"} ${className}`}>{children}</td>;
}
            ...
            {displayRows.map((r) => {
              ...
              return (
                ...
                >
                  ...
                  <Td suppressHydrationWarning className="text-muted-foreground text-xs uppercase">
                    {new Date(r.data).toLocaleDateString("pt-BR", { month: "short", year: "2-digit", timeZone: "UTC" }).replace(" de ", " ")}
                  </Td>
